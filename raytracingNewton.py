import numpy as np
from classesNewton import  Illum, Sphere, Newton, normalize, Light, Ray, Cone
        ...
        for bounce in range(5):
            ...
            if intersection == 1:
               ...
               # normal = normalize(betterPoint - functions[idFunction].center())
                normal = np.array([functions[idFunction].dx(betterPoint), functions[idFunction].dy(betterPoint), functions[idFunction].dz(betterPoint)])
